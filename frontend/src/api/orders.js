...
export async function updateOrderStatus(id, status) {
  console.log(`🔄 Frontend: Actualizando estado del pedido ${id} a "${status}"`);
  ...
  if (!res.ok) {
    console.error(`❌ Frontend: Error actualizando estado: ${res.status} ${res.statusText}`);
    throw new Error(`Error ${res.status}: ${res.statusText}`);
  }
  
  const result = await res.json();
  console.log(`✅ Frontend: Estado actualizado exitosamente`);
  return result;
}
